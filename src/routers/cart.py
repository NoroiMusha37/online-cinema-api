...
async def get_user_cart(
    ...
    return paginate(
        items=cart_items, count=count, page=page, size=size, path="/cart/me/"
    )
...
async def add_to_cart(
    ...
    new_cart_item = await cart_crud.create_cart_item(
        cart_item_in=cart_item_in, user_id=current_user.id, session=session
    )
    if not new_cart_item:
        return {
            "message": "This movie is already in the cart "
...
async def remove_from_cart(
    ...
    await cart_crud.delete_cart_item(
        user_id=current_user.id, cart_item_id=item_id, session=session
    )
...
async def clear_cart(
    ...
    await cart_crud.clear_cart(user_id=current_user.id, session=session)
...
async def get_certain_user_cart(
    ...
    user = await user_crud.get_user_by_id(user_id=user_id, session=session)
    if not user:
        raise HTTPException(
    ...
    cart_items, count = await cart_crud.get_cart_items(
        user_id=user_id, page=page, size=size, session=session
    )
