...
async def seed():
    async with SessionLocal() as session:
        ...
        admin_group = await get_or_create(
            session, UserGroup, name=UserGroupEnum.ADMIN
        )
        user_group = await get_or_create(
            session, UserGroup, name=UserGroupEnum.USER
        )
        ...
        for movie in movies_list[:10]:
            ...
            if random.choice([True, False]):
                session.add(
                    MovieLike(user_id=user1.id, movie_id=movie.id, like=True)
                )
            ...
            if random.choice([True, False]):
                stmt = insert(user_favorites).values(
                    user_id=user1.id, movie_id=movie.id
                )
